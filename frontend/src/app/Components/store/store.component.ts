...
export class StoreComponent implements OnInit {
  ...
  addStore(storeName: NgModel) {
    // Check if the store name is valid
    if (storeName.invalid) {
      storeName.control.markAsTouched(); // Mark the field as touched to show the error message
      return; // Prevent further execution if the form is invalid
    }

    // Proceed with adding the store
    this.storeService.addStore(this.selectedStore).subscribe(
      (store) => {
        this.stores.push(store);
        this.selectedStore = new Store();
